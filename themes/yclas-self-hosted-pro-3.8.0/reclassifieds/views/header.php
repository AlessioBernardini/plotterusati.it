    ...
    <div class="container">
        <div class="navbar-header">
            ...
            <ul class="navbar-btn pull-right hidden-md hidden-lg">
                <ul class="hidden-sm hidden-xs"><?=View::factory('widget_login')?></ul>
                <?if ((Core::config('advertisement.only_admin_post')!=1) OR (Core::config('advertisement.only_admin_post')==1 AND Auth::instance()->logged_in() AND (Auth::instance()->get_user()->is_admin() OR Auth::instance()->get_user()->is_moderator()))):?>
                    <li>
                    	<a class="btn btn-success btn-mini" href="<?=Route::url('post_new')?>">
        ...
        <div class="collapse navbar-collapse" id="mobile-menu-panel">
        		...
        		<div class="hidden-xs hidden-sm" id="post-new">
	      			<?if ((Core::config('advertisement.only_admin_post')!=1) OR (Core::config('advertisement.only_admin_post')==1 AND Auth::instance()->logged_in() AND (Auth::instance()->get_user()->is_admin() OR Auth::instance()->get_user()->is_moderator()))):?>
			       		<a class="btn btn-success" href="<?=Route::url('post_new')?>">
			        	<i class="fa fa-pencil"></i> <?=_e('Publish new')?></a>
			        <?endif?>
			      </div>
	    ...
	    <ul>
	        ...
	        <?foreach($cats as $c ):?>
	            <?if($c['id_category_parent'] == 1 && $c['id_category'] != 1):?>
	                <li class="drop <?=($c['id_category'] == $cat_parent OR $c['id_category'] == $cat_id)?'active current':''?>">
	                    ...
	                    <?if($chi['id_category_parent'] == $c['id_category']):?>
	                        <li class="<?=($chi['id_category'] == $cat_id)?'active current':''?>" >
