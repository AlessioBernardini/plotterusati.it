...
<div class="page-header text-center">
    <h1><?= _e('User Profile')?></h1>
</div>

<div id="user_profile_info" class="container">
    <div class="row">
        <div class="col-md-3">
                                    ...
                                    <a title="<?=HTML::chars($ad->title)?>" href="<?=Route::url('ad', array('controller'=>'ad','category'=>$ad->category->seoname,'seotitle'=>$ad->seotitle))?>">
                                        <figure>
                                            <?if($ad->get_first_image() !== NULL):?>
                                                  <img src="<?=Core::imagefly($ad->get_first_image('image'),150,150)?>" class="img-responsive center-block" alt="<?=HTML::chars($ad->title)?>" />
                                              <?elseif(( $icon_src = $ad->category->get_icon() )!==FALSE ):?>
                                                  <img src="<?=Core::imagefly($icon_src,150,150)?>" class="img-responsive center-block" alt="<?=HTML::chars($ad->title)?>" />
                                              <?elseif(( $icon_src = $ad->location->get_icon() )!==FALSE ):?>
                                                  <img src="<?=Core::imagefly($icon_src,150,150)?>" class="img-responsive center-block" alt="<?=HTML::chars($ad->title)?>" />
                                              <?else:?>
                                                  <img data-src="holder.js/150x150?<?=str_replace('+', ' ', http_build_query(array('text' => $ad->category->translate_name(), 'size' => 12, 'auto' => 'yes')))?>" class="img-responsive center-block" alt="<?=HTML::chars($ad->title)?>">
                                              <?endif?>
                                        </figure>
                                    </a>
                                </div>

                                <p><strong><?=_e('Description')?>: </strong><?=Text::limit_chars(Text::removebbcode($ad->description))?><p>
