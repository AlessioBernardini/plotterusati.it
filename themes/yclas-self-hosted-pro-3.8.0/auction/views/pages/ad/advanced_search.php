...
<?=Form::errors()?>
<h1><?=_e('Search')?></h1>
<div class="advanced_search_box clearfix">
    <?= FORM::open(Route::url('search'), array('class'=>'form-inline', 'method'=>'GET', 'action'=>''))?>
        <fieldset>
            <div class="clearfix">
                <div class="form-group">
                    <?= FORM::label('advertisement', ((core::config('general.search_by_description') == TRUE)?_e('Keywords'):_e('Advertisement Title')), array('class'=>'', 'for'=>'advertisement'))?>
                    <div class="control mr-30">
                        <?if(Core::config('general.algolia_search') == 1):?>
                            <?=View::factory('pages/algolia/autocomplete_ad')?>
                        <?else:?>
                            <input type="text" id="title" name="title" class="form-control" value="<?=HTML::chars(core::get('title'))?>" placeholder="<?=((core::config('general.search_by_description') == TRUE)?__('Keywords'):__('Title'))?>">
                        <?endif?>
                    </div>
                </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="control mr-30">
                            ...
                            <?if ( ! core::config('general.search_multi_catloc')) :?>
                                <option value=""><?=_e('Category')?></option>
                            <?endif?>
                ...
                <?if(core::config('advertisement.location') != FALSE AND core::count($locations) > 1):?>
                    <div class="form-group">
                        ...
                        <div class="control mr-30">
                            ...
                            <?if ( ! core::config('general.search_multi_catloc')) :?>
                                <option value=""><?=_e('Location')?></option>
                            <?endif?>
            ...
            <div class="form-group hidden-lg hidden-md hidden-sm">
                <label>&nbsp;</label>
                <div class="control mr-30">
                    <?= FORM::button('submit', _e('Search'), array('type'=>'submit', 'class'=>'btn btn-primary', 'action'=>Route::url('search')))?>
                </div>
            </div>
