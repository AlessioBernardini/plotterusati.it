			...
			<div class="<?=(Theme::get('sidebar_position')!='none')?'col-lg-9 col-md-9 col-sm-12 col-xs-12':'col-xs-12'?> <?=(Theme::get('sidebar_position')=='left')?'pull-right':'pull-left'?>">
					...
					<div class="listing_filter">
						...
						<?if(core::config('general.auto_locate')):?>
							...
							class="btn btn-sm btn-base-dark <?=core::request('userpos') == 1 ? 'active' : NULL?>"
								...
								data-marker-title="<?=_e('My Location')?>"
								data-marker-error="<?=_e('Cannot determine address at this location.')?>"
								data-href="?<?=http_build_query(['userpos' => 1] + Request::current()->query())?>">
								<i class="glyphicon glyphicon-map-marker"></i> <?=sprintf(_e('%s from you'), i18n::format_measurement(Core::config('advertisement.auto_locate_distance', 1)))?>
							</button>
						<?endif?>
						...
						<div class="sort_opts btn-group" id="listgrid" data-default="<?=Theme::get('listgrid')?>">
				...
				<div class="ad_listings">
					<ul class="ad_list list clearfix">
						...
						<li class="<?=(core::cookie('list/grid')==0)?'col-lg-4 col-md-4 col-sm-4 col-xs-12 ad_griditem':'ad_listitem'?> clearfix featured_ad">
							<span class="feat_marker"><i class="glyphicon glyphicon-bookmark"></i></span>
						<?else:?>
						<li class="<?=(core::cookie('list/grid')==0)?'col-lg-4 col-md-4 col-sm-4 col-xs-12 ad_griditem':'ad_listitem'?> clearfix">
						<?endif?>
							<div class="ad_inner col-xs-12">
									...
									<div class="ad_photo_inner">
										<a title="<?=HTML::chars($ad->title)?>" href="<?=Route::url('ad', array('controller'=>'ad','category'=>$ad->category->seoname,'seotitle'=>$ad->seotitle))?>">
											<?if($ad->get_first_image() !== NULL):?>
												<img class="img-responsive" src="<?=Core::imagefly($ad->get_first_image('image'),200,200)?>" alt="<?=HTML::chars($ad->title)?>">
											<?else:?>
								...
								<div class="ad_details">
									<div class="ad_details_inner">
				                        ...
				                        <?if ((Core::config('advertisement.reviews')==1) AND (round($ad->rate,1) > 0)):?>
				                        	<div class="pull-left">
										...
										<? foreach ($ad->custom_columns(TRUE) as $name => $value) : ?>
											<? if ($value == 'checkbox_1') : ?>
												<p class="ad_meta clearfix"><b><?= $name ?></b>: <i class="fa fa-check"></i></p>
											<? elseif ($value == 'checkbox_0') : ?>
												<p class="ad_meta clearfix"><b><?= $name ?></b>: <i class="fa fa-times"></i></p>
											<? elseif (is_string($name)) : ?>
												<p class="ad_meta clearfix"><b><?= $name ?></b>: <?= $value ?></p>
											<? else : ?>
												<p class="ad_meta clearfix"><?= $value ?></p>
											<? endif ?>
										<? endforeach ?>

										<?if(core::config('advertisement.description')!=FALSE):?>
											<p class="ad_desc">
												<?=(core::cookie('list/grid')==1)?Text::limit_chars(Text::removebbcode($ad->description), 255, NULL, TRUE) : Text::limit_chars(Text::removebbcode($ad->description), 255, NULL, TRUE)?>
												<?foreach ($ad->custom_columns(TRUE) as $name => $value):?>
							                        <?if($value=='checkbox_1'):?>
							                            <br><b><?=$name?></b>: <i class="fa fa-check"></i>
							                        <?elseif($value=='checkbox_0'):?>
							                            <br><b><?=$name?></b>: <i class="fa fa-times"></i>
							                        <?elseif(is_string($name)):?>
							                            <br><b><?=$name?></b>: <?=$value?>
							                        <?else:?>
							                            <br><?=$value?>
								...
								<div class="ad_buttons col-xs-12">
												...
												<div class="modal-content">
													<div class="modal-body">
														...
														<ul class="ad_controls_list">
															<li><a class="btn btn-success" href="<?=Route::url('oc-panel', array('controller'=>'myads','action'=>'update','id'=>$ad->id_ad))?>"><i class="glyphicon glyphicon-edit"></i> <?=_e("Edit");?></a></li>
															<li><a class="btn btn-warning" href="<?=Route::url('oc-panel', array('controller'=>'ad','action'=>'deactivate','id'=>$ad->id_ad))?>" onclick="return confirm('<?=_e('Deactivate?')?>');"><i class="glyphicon glyphicon-off"></i> <?=_e("Deactivate");?></a></li>
															<li><a class="btn btn-danger" href="<?=Route::url('oc-panel', array('controller'=>'ad','action'=>'spam','id'=>$ad->id_ad))?>" onclick="return confirm('<?=_e('Spam?')?>');"><i class="glyphicon glyphicon-fire"></i> <?=_e("Spam");?></a></li>
															<li><a class="btn btn-danger" href="<?=Route::url('oc-panel', array('controller'=>'ad','action'=>'delete','id'=>$ad->id_ad))?>" onclick="return confirm('<?=_e('Delete?')?>');"><i class="glyphicon glyphicon-remove"></i> <?=_e("Delete");?></a></li>
														</ul>
													</div>
												</div>
											</div>
											...
											<div class="modal-dialog">
												<div class="modal-content">
													<div class="modal-body">
														...
														<ul class="ad_controls_list">
															<li><a class="btn btn-success" href="<?=Route::url('oc-panel', array('controller'=>'myads','action'=>'update','id'=>$ad->id_ad))?>"><i class="glyphicon glyphicon-edit"></i> <?=_e("Edit");?></a></li>
															<li><a class="btn btn-warning"  href="<?=Route::url('oc-panel', array('controller'=>'myads','action'=>'deactivate','id'=>$ad->id_ad))?>" onclick="return confirm('<?=_e('Deactivate?')?>');"><i class="glyphicon glyphicon-off"></i> <?=_e("Deactivate");?></a></li>
														</ul>
													</div>
												</div>
											</div>
									...
									<?else:?>
										<span class="ad_price na">
											<a class="add-transition" title="<?=HTML::chars($ad->title)?>" href="<?=Route::url('ad', array('controller'=>'ad','category'=>$ad->category->seoname,'seotitle'=>$ad->seotitle))?>"><?=_e('Check Listing');?></a>
										</span>
									<?endif?>
								</div>
							</div>
				...
				<?else:?>
					...
					<div class="no_results text-center">
						...
						<?if (Core::config('advertisement.only_admin_post')!=1):?>
							<a class="btn btn-base-dark" title="<?=_e('New Advertisement')?>"
