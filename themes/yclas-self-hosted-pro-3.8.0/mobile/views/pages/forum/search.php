...
<div class="page-header">
    ...
    <?if (!Auth::instance()->logged_in()):?>
        <a class="btn btn-info new-topic" data-toggle="modal" data-dismiss="modal" 
            href="<?=Route::url('oc-panel',array('directory'=>'user','controller'=>'auth','action'=>'login'))?>#login-modal">
    <?else:?>
        <a class="btn btn-info new-topic" href="<?=Route::url('forum-new')?>">
    <?endif?>
        <?=_e('New Topic')?></a>
    </div>
    <?=View::factory('pages/forum/search-form')?>
    <h1 class="forum-title pull-left"><?=_e('Search')?> <?=HTML::chars(core::get('search'))?></h1>
...
<?else:?>
    <h2 class="help-block"><?=_e('Nothing found, sorry!')?></h2>
