<div class="page-header">
    <h1><?=$topic->title?></h1>
    <span class="label label-info"><?=$topic->user->name?> <?=$topic->user->is_verified_user()?> <?=Date::fuzzy_span(Date::mysql2unix($topic->created))?></span>
    <?if($previous->loaded()):?>
                ...
                <p>
                    <?if (in_array('profile', Route::all())) :?>
                        <a href="<?=Route::url('profile', array('seoname'=>$topic->user->seoname)) ?>">
                            <?=$topic->user->name?> <?=$topic->user->is_verified_user()?>
                        </a>
                    <?else :?>
                        <?=$topic->user->name?> <?=$topic->user->is_verified_user()?>
                    <?endif?>
    ...
    <div class="col-md-9 span6">
        ...
        <?if (Auth::instance()->logged_in()):?>
            <a  class="btn btn-primary reply" href="#reply_form"><?=_e('Reply')?></a>
        <?else:?>
            <a class="btn btn-primary reply" data-toggle="modal" data-dismiss="modal" href="<?=Route::url('oc-panel',array('directory'=>'user','controller'=>'auth','action'=>'login'))?>#login-modal">
                <?=_e('Reply')?>
            </a>
        <?endif?>
    </div>
...
<?foreach ($replies as $reply):?>
                ...
                <p>
                    <?if (in_array('profile', Route::all())) :?>
                        <a href="<?=Route::url('profile', array('seoname'=>$reply->user->seoname)) ?>">
                            <?=$reply->user->name?> <?=$reply->user->is_verified_user()?>
                        </a>
                    <?else :?>
                        <?=$reply->user->name?> <?=$reply->user->is_verified_user()?>
                    <?endif?>
    ...
    <?endif?>
        ...
        <a  class="btn btn-xs btn-primary reply" href="#reply_form"><?=_e('Reply')?></a>
    </div>
...
<h3><?=_e('Reply')?></h3>
  <?php if ($errors): ?>
    ...
    <div class="form-group control-group">
        <div class="col-md-12">
            <textarea name="description" rows="10" class="form-control input-xxlarge" style="height:250px;" required><?=core::post('description',_e('Reply here'))?></textarea>
        </div>
    </div>
