...
<?if(Auth::instance()->logged_in()):?>
	<?if (core::config('general.messaging') AND $messages = Model_Message::get_unread_threads(Auth::instance()->get_user())) :?>
		<div class="btn-group" role="group">
			<?if (($messages_count = $messages->count_all()) > 0) :?>
				<a class="btn btn-base-light widget_notification"
	...
	<?elseif ($ads = Auth::instance()->get_user()->contacts() AND core::count($ads) > 0) :?>
		<div class="btn-group" role="group">
			<a class="btn dropdown-toggle btn-base-light widget_notification" data-toggle="dropdown" href="#" id="contact-notification" data-url="<?=Route::url('oc-panel', array('controller'=>'profile', 'action'=>'notifications'))?>">
