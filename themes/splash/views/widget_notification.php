...
<?if(Auth::instance()->logged_in()):?>
    <?if (core::config('general.messaging') AND $messages = Model_Message::get_unread_threads(Auth::instance()->get_user())) :?>
        <li class="dropdown">
            <?if (($messages_count = $messages->count_all()) > 0) :?>
                <a
                    class="primary-btn color-secondary header-btn"
    ...
    <?elseif ($ads = Auth::instance()->get_user()->contacts() AND core::count($ads) > 0) :?>
        <li class="dropdown">
            <a class="primary-btn color-secondary header-btn contact-notification" id="contact-notification" href="#" data-toggle="dropdown" data-url="<?=Route::url('oc-panel', array('controller'=>'profile', 'action'=>'notifications'))?>">
