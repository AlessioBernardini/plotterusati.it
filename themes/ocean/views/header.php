...
<header class="navbar navbar-inverse <?=(strtolower(Request::current()->controller())!=='faq' AND Theme::get('fixed_toolbar')==1)?'navbar-fixed-top':''?>">
    <div class="container">
            ...
            <ul class="nav navbar-nav">
                ...
                <?else:?>
                                                    ...
                                                    <li>
                                                        <a title="<?=HTML::chars($chi['translate_name'])?>" href="<?=Route::url('list', array('category'=>$chi['seoname'],'location'=>$loc_seoname))?>">
                                                            <?if (Theme::get('category_badge') != 1) : ?>
                                                                <span class="pull-right badge badge-success hidden-xs"><?=number_format($chi['count'])?></span>
                                                            <?endif?>
                    ...
                    <?if (core::config('general.faq')==1):?>
                        <?=Theme::nav_link(_e('FAQ’s'),'faq','glyphicon glyphicon-question-sign','index','faq')?>
                    <?endif?>
