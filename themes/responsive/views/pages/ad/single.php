...
<?if ($ad->status != Model_Ad::STATUS_PUBLISHED && $permission === FALSE):?>
    <div class="page-header">
        <h3><?= _e("This advertisement doesn't exist, or is not yet published")?></h3>
    </div>
<?else:?>
    <?=Form::errors()?>
    <div class="page-header">
        <div class="pull-right favorite" id="fav-<?=$ad->id_ad?>">
            <?if (Auth::instance()->logged_in()):?>
                ...
                <a data-id="fav-<?=$ad->id_ad?>" class="<?=($fav)?'remove':'add'?>-favorite" title="<?=__('Add to Favorites')?>" href="<?=Route::url('oc-panel', array('controller'=>'profile', 'action'=>'favorites','id'=>$ad->id_ad))?>">
        ...
        <?if((core::config('payment.pay_to_go_on_top') > 0
            ...
            <div id="recomentadion" class="well recomentadion clearfix">
                <?if(core::config('payment.pay_to_go_on_top') > 0 && core::config('payment.to_top') != FALSE):?>
                    ...
                    <a class="btn btn-mini btn-primary" type="button" href="<?=Route::url('default', array('action'=>'to_top','controller'=>'ad','id'=>$ad->id_ad))?>"><?=_e('Go Top!')?></a>
                <?endif?>
                <?if(core::config('payment.to_featured') != FALSE AND $ad->featured < Date::unix2mysql()):?>
                    ...
                    <a class="btn btn-mini btn-primary" type="button" href="<?=Route::url('default', array('action'=>'to_featured','controller'=>'ad','id'=>$ad->id_ad))?>"><?=_e('Go Featured!')?></a>
                <?endif?>
            </div>
        <?endif?>
    <?endif?>
    ...
    <div class="well">
        ...
        <?if($images):?>
            <div class="row single-prettyPhoto">
                <?foreach ($images as $path => $value):?>
                    <?if( isset($value['thumb']) AND isset($value['image']) ):?>
                        <div class="col-md-2">
                            <a href="<?=$value['image']?>" data-gallery>
                                <img src="<?=Core::imagefly($value['image'],220,220)?>" class="center-block img-thumbnail img-responsive">
                            </a>
                        </div>
                    <?endif?>
                <?endforeach?>
        ...
        <?endif?>
            ...
            <div class="well">
            <?if ($ad->price>0):?>
                <span class="label label-warning"><?= _e('Price');?> : <span class="price-curry"><?=i18n::money_format( $ad->price, $ad->currency())?></span></span>
            <?endif?>
            <?if ($ad->price==0 AND core::config('advertisement.free')==1):?>
                <span class="label label-warning"><?= _e('Price');?> : <?=_e('Free');?></span>
            <?endif?>
            <?if ($ad->id_location != 1):?>
                <span class="label label-default"><?=$ad->location->translate_name() ?></span>
            <?endif?>
            <a class="label label-default" href="<?=Route::url('profile',  array('seoname'=>$ad->user->seoname))?>"><?=$ad->user->name?> <?=$ad->user->is_verified_user();?></a>
