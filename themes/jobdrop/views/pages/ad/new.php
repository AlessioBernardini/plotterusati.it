...
<div class="page-header text-center">
    <h1 class="h1"><?=_e('Publish new advertisement')?></h1>
</div>
<section id="main">
                ...
                <div class="contact no-gutter">
                    <div class="col-xs-12">
                        <?if (Theme::get('premium')==1 AND core::count($providers = Social::get_providers())>0 AND !Auth::instance()->get_user()):?>
                            ...
                            <?endforeach?>
                        <?endif?>
                        <?= FORM::open(Route::url('post_new',array('controller'=>'new','action'=>'index')), array('class'=>'form-horizontal post_new standard-frm publish-new-frm', 'id'=>'publish-new', 'enctype'=>'multipart/form-data'))?>
                            ...
                            <?= FORM::input('title', Request::current()->post('title'), array('placeholder' => __('Title *'), 'class' => 'form-control input', 'id' => 'title', 'required'))?>
                            <div class="clearfix"></div>
                            <!-- category select -->
                            ...
                            <?endif?>
                                <?if ( ! isset($form_show['description']) OR $form_show['description'] != FALSE):?>
                             ...
                             <?if($form_show['phone'] != FALSE):?>
                                    ...
                                    <div>
                                        <?if (Auth::instance()->get_user()):?>
                                            <?=FORM::input('phone', Auth::instance()->get_user()->phone, array('class'=>'input form-control', 'id'=>'phone', 'data-country' => core::config('general.country')))?>
                                        <?else:?>
                                            <?=FORM::input('phone', Request::current()->post('phone'), array('class'=>'input form-control', 'id'=>'phone', 'data-country' => core::config('general.country')))?>
                                        <?endif?>
                                    </div>
                             <?endif?>
                             <?if($form_show['address'] != FALSE):?>
                                ...
                                <?if(core::config('advertisement.map_pub_new')):?>
                                    ...
                                    <?endif?>
                                <?else:?>
                                    <?= FORM::input('address', $address_default_value, array('class'=>'form-control  input', 'id'=>'address', 'placeholder'=>__('Address')))?>
                                <?endif?>
                                <?if(core::config('advertisement.map_pub_new')):?>
                             ...
                             <?if($form_show['price'] != FALSE):?>
                                 ...
                                 <?= FORM::input('price', Request::current()->post('price'), array('placeholder' => html_entity_decode(html_entity_decode(i18n::money_format(1))), 'class' => 'form-control input', 'id' => 'price', 'type'=>'text', 'data-error' => __('Please enter only numbers.'), 'data-decimal_point' => i18n::get_decimal_point()))?>
                                 </div>
                             <?endif?>
                             <?if(core::config('payment.stock')):?>
                                 <?= FORM::label('stock', _e('In Stock'), array('class'=>'', 'for'=>'stock'))?>
                                 <div class="input-prepend">
                                 <?= FORM::input('stock', Core::post('stock', 1), array('placeholder' => '10', 'class' => 'form-control input', 'id' => 'stock', 'type'=>'text'))?>
                                 </div>
                             <?endif?>
                             <?if($form_show['website'] != FALSE):?>
                                 <?= FORM::label('website', _e('Website'), array('class'=>'', 'for'=>'website'))?>
                                 <?= FORM::input('website', Request::current()->post('website'), array('placeholder' => core::config("general.base_url"), 'class' => 'form-control input', 'id' => 'website'))?>
                             <?endif?>
                             <?if (!Auth::instance()->get_user()):?>
                                <?= FORM::label('name', _e('Name'), array('class'=>'', 'for'=>'name'))?>
                                <?= FORM::input('name', Request::current()->post('name'), array('class'=>'form-control input', 'id'=>'name', 'required', 'placeholder'=>__('Name')))?>
                                <?= FORM::label('email', (core::config('payment.paypal_seller')==1)?_e('Paypal Email'):_e('Email'), array('class'=>'', 'for'=>'email'))?>
                                <?= FORM::input('email', Request::current()->post('email'), array('class'=>'form-control input',
                                    'id'=>'email',
                                    'type'=>'email',
                                    'required',
                             ...
                             <?if ($form_show['captcha'] != FALSE):?>
                                ...
                                <?endif?>
                             <?endif?>
                             <?= FORM::button('submit_btn', _e('Publish new'), array('type'=>'submit', 'id' => 'publish-new-btn', 'class'=>'primary-btn color-primary btn', 'data-swaltitle' => __('Are you sure?'), 'data-swaltext' => __('It looks like you have been about to publish a new advertisement, if you leave before submitting your changes will be lost.'), 'class'=>'btn btn-primary', 'action'=>Route::url('post_new',array('controller'=>'new','action'=>'index'))))?>
                             <?if (!Auth::instance()->get_user()):?>
