...
<?=Form::errors()?>
<div id="users_search_form" class="well recomentadion def-size-form clearfix <?=(Theme::get('dark_listing'))?'dark_listing':''?>">
...
<div class="row" id="users">
    <?$i = 1; foreach($users as $user ):?>
        <div class="col-sm-4" style="min-height:350px">
            <div class="thumbnail <?=(Theme::get('dark_listing'))?'dark_listing':''?>">
                <a title="<?=HTML::chars($user->name)?>" href="<?=Route::url('profile',  array('seoname'=>$user->seoname))?>">
                    <img class="img-responsive" src="<?=Core::imagefly($user->get_profile_image(),250,250)?>" alt="<?=HTML::chars($user->name)?>">
                </a>
                <div class="caption">
                    <h3>
                        <a title="<?=HTML::chars($user->name)?>" href="<?=Route::url('profile',  array('seoname'=>$user->seoname))?>">
