...
<div class="panel-body">
	<?foreach($widget->ads as $ad):?>
		...
		<div class="well <?=(get_class($widget)=='Widget_Featured')?'featured-custom-box':''?> <?=(Theme::get('dark_listing'))?'dark_listing':''?>">
			<div class="featured-sidebar-box">
				<?if($ad->get_first_image() !== NULL):?>
					<div class="picture pull-right col-xs-12 <?=($widget->placeholder=='header')?'col-md-3':'col-md-12'?> <?=($widget->placeholder=='footer')?'col-sm-12':'col-sm-3'?>">
						<a class="pull-right" title="<?=HTML::chars($ad->title);?>" alt="<?=HTML::chars($ad->title);?>" href="<?=Route::url('ad', array('controller'=>'ad','category'=>$ad->category->seoname,'seotitle'=>$ad->seotitle))?>">
							<figure>
								<img src="<?=$ad->get_first_image()?>" class="img-responsive">
							</figure>
						</a>
					</div>
				<?else:?>
